...
#           print(example)
          ...
          if len(example_item) > 0 :
            word.sentence = example_item[0]
            word.translation = example_item[1]
            
#     return word #(spell, pron, accent, picker[0] , picker[1]) + tuple(example_item)


#提取要处理單詞
def goi_excel(wbname, sheetname='Sheet1' ):
    """
    ...
    for row in range(2, sheet.max_row + 1):  # 第2行開始
      ...
      if not (word.accent and word.word_class):
#         print(word.accent , word.word_class)
        example_item.append(word)
     
    print(len(example_item))
    
    return example_item


#保存要处理單詞
def create_to_excel(wbname, words_data, sheetname='Sheet1' ):
    """
        将制定的信息保存到新建的excel表格中;
    ...
    # 将数据data写入excel表格中;
    print("正在写入数据........")
    for row, item in enumerate(words_data):  # data发现有4行数据， item里面有三列数据;
      sheet.cell(row+2, 1, item.spell) 
      sheet.cell(row+2, 2, item.pron)
      sheet.cell(row+2, 3, item.accent)
      sheet.cell(row+2, 4, item.word_class)
      sheet.cell(row+2, 5, item.meanings)
      sheet.cell(row+2, 6, item.sentence)
      sheet.cell(row+2, 7, item.translation)
...
def save_to_excel(wbname, sheetname='Sheet1' ):
    ...
    for row in range(2, sheet.max_row + 1):  # 第2行開始
      ...
      if spell and pron :
        ...
        if not (accent and word_class and meanings and sentence and translation):
  #         print(word.accent , word.word_class)
          search_moji_dict(word)
          
          # 檢測空        
          if not (word.spell and word.pron and word.accent and word.word_class and word.meanings and word.sentence and word.translation) :
            print(word.spell, word.pron)
            search_hj_dict(word)
  
          if not (word.spell == spell and word.pron == pron and word.accent == accent and word.word_class == word_class and word.meanings == meanings and word.sentence == sentence and word.translation == translation):
...
#   print(len(set(goi_excel("D:/单词.xlsx","片仮名")) - set(goi_excel("D:/单词1.xlsx","1、2級")) ))

#     create_to_excel('d:/hello.xlsx', goi_excel("D:/单词.xlsm","N2級"))
    save_to_excel("D:/hello.xlsx","N2核心")
